...
def create_doctor():
    ...
    try:
        master_conn.execute(
            "INSERT INTO doctors (username, password, clinic_name, status, account_type, expiry_date, created_at) VALUES (?, ?, ?, 'active', ?, ?, ?)",
            (u, hashed_p, c, at, exp, datetime.date.today().isoformat())
        )
        master_conn.commit()
